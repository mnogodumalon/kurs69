    ...
    async def generate_typescript(args):
        ...
        try:
            ...
            if crud_scaffolds:
                ...
                response_text += f"\n{'='*60}"
                response_text += "\n"
                response_text += "\n1. WRITE ONCE: Write/edit each file ONCE. Do NOT write a file, read it back, then rewrite."
                response_text += "\n   Plan your design system FULLY before touching index.css."
                response_text += "\n   Plan DashboardOverview FULLY before writing it."
                response_text += "\n"
                response_text += "\n2. DO NOT READ: All key files are included below. Do NOT use the Read tool on ANY of them."
                response_text += "\n   Also do NOT Read CRUD pages, dialogs, Layout.tsx, ConfirmDialog, StatCard, or PageShell."
                response_text += "\n   They are COMPLETE and must NOT be rewritten."
                response_text += "\n"
                response_text += "\n3. CSS FORMAT: index.css uses oklch() colors and @theme inline — NOT hsl()!"
                response_text += "\n   EXTEND the existing :root block with Edit tool — do NOT rewrite from scratch."
                response_text += "\n   CSS import order MUST be: @import url(...) FIRST, then @import \"tailwindcss\", then @import \"tw-animate-css\"."
                response_text += "\n   Do NOT reorder or change the existing @import lines."
                response_text += "\n"
                response_text += "\nSTEPS (in order):"
            ...
            # ═══════════════════════════════════════════════════════
            # FILE CONTENTS — reference material (after instructions)
            # ═══════════════════════════════════════════════════════
            ...
            if css_path.exists():
                ...
                response_text += f"\n\n{'='*60}\n📄 src/index.css (EXISTING TEMPLATE — use Edit to EXTEND, not Write to replace!)\n{'='*60}\n{css_header}\n/* ... more :root variables follow ... */"
